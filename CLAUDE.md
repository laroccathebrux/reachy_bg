...
| Reachy daemon | `uv run reachy-mini-daemon` (it lives in `.venv/bin`, never on the PATH), `http://127.0.0.1:8000` | Started per session **from iTerm**, in the repository directory; robot on USB `/dev/cu.usbmodem*`. |
