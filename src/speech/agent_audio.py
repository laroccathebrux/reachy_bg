...
Echo gate: the Mac microphone has no echo cancellation and hears the robot at the same level
as a player, so while the speaker is busy (plus a short tail) microphone audio is *held back*
in a small ring buffer instead of going to the agent; the local taps still receive it. When
the local echo-aware check decides a player is really talking over the robot,
...
class RobotAudioInterface:
    ...
    def __init__(self, input_device: str = AUDIO_INPUT_DEVICE, output_device: str = AUDIO_OUTPUT_DEVICE):
        ...
        self._spoke_until = 0.0  # monotonic time until which the gate stays closed
        self._held: deque[bytes] = deque(maxlen=8)  # 8 x 250 ms of gated microphone audio
        self.gated_frames = 0
    ...
    def _write(self, block: np.ndarray) -> None:
        if self._out is None:
            return
        self.speaking = True
        self._spoke_until = time.monotonic() + OUTPUT_BLOCK / SAMPLE_RATE + self.gate_tail_s
...
__all__ = ["RobotAudioInterface", "resolve_output_device", "rms_dbfs", "SAMPLE_RATE"]
